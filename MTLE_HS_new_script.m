...
for i = 1:(size(s_peaks,1));
    ...
    unix(['source ~/.bash_profile ; tckgen -algorithm Tensor_Prob -force -nthreads 14 -select 10000 ' int_output filesep s_dwis(i).name ' ' final_output filesep basename 'TP_L_phC.tck  -seed_gmwmi ' ...
        VOIs_comp(i).voi_folder filesep VOIs_comp(i).voi_name{5}   ' -mask ' int_output filesep s_masks(i).name  ' -act ' final_output filesep dwi_p5tt(i).name ...
